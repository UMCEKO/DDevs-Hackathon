import {CommandInteraction, CommandInteractionOption, Embed, EmbedBuilder, SlashCommandBuilder, User} from "discord.js";
import * as url from "url";
import {db, Group, User as DBUser, Role} from "../fn/dbfn";
...
module .exports = {
    ...
    execute: async (interaction, dbuser, userRole, dbGroup)=>{
        ...
        if (targetUser){
            let tuser = await db.getUser(targetUser.id)
            let trole = await db.getRoleOf(targetUser.id)
            await interaction.reply({embeds: [getProfile(targetUser, tuser, trole, dbGroup, interaction.locale)]})
            return
        }
        else {
            await interaction.reply({embeds: [getProfile(interaction.user, dbuser, userRole, dbGroup, interaction.locale)]})
            return
        }
    }
} as IDiscordCommand
...
const getProfile = (user: User, dbuser: DBUser, userRole: Role, dbgroup: Group | null, locale: string | undefined): EmbedBuilder => {
    if (locale === "tr"){
