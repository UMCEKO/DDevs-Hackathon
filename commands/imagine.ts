...
function optionifyStyle(...styles: {name: string, prompt: string, negative_prompt: string}[]): APIApplicationCommandOptionChoice<string>[]{
    let optionifiedStyleArr: optionifiedStyle[] = []
...
module .exports = {
    ...
    execute: async (interaction, dbuser, userRole, dbGroup)=>{
        let model: "anime" | "sdxl" = (interaction.options.getString("model") as "anime" | "sdxl" | null) || "sdxl"
        let negativePr = interaction.options.getString("negative-prompt") || ""
        let positivePr = interaction.options.getString("prompt", true)
        let seed = interaction.options.getInteger("seed") || -1
        let widthInput = interaction.options.getInteger("width")
        let heightInput = interaction.options.getInteger("height")
        ...
        if (model === "anime"){
            ...
            if (aspect.width * aspect.height > 1024 * 1024){
                let ar = aspect.width/aspect.height
                aspect = {
                    width: 1024*ar,
                    height: 1024/ar
                }
        ...
        // }
        if (!channel.nsfw){
            let modResponse =await YildizAI.moderations.create({
