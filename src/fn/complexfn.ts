import {addQueue, getQueue, Role} from "./dbfn";


class DatabaseParams {
    constructor(imgReq: ImagineRequest, positive_wos: string, negative_wos: string, styles: string[]) {
...
async function imagine(generationOptions: imagineOptions, userid: string, chatid: string, messageid: string, groupid: string|null, platform: "whatsapp" | "discord" | "telegram"){
    ...
    if (generationOptions.aspect === undefined) generationOptions.aspect = {width: generationOptions.anime ? 512 : 1024, height: generationOptions.anime ? 768 : 1024}
    ...
    function stylisePrompt(prompt: PromptStruct){
        ...
        if (prompt.styles.length === 0){
            ...
            for (let userStyle of prompt.styles){
                if (!SDXLStyles[userStyle as keyof typeof SDXLStyles]) {
                    throw "Invalid Style"
                }
                let style = SDXLStyles[userStyle as keyof typeof SDXLStyles];
                positive = style.prompt.replaceAll('{prompt}', positive)
                negative = negative + style.negative_prompt
            }
            return {
    ...
    try {
        if(!generationOptions.anime){
            stylisedPrompt = stylisePrompt({positive: generationOptions.positivePrompt, styles: generationOptions.styles, negative: generationOptions.negativePrompt || ""})
        }
            ...
            stylisedPrompt = {negative: generationOptions.negativePrompt || "", positive: generationOptions.positivePrompt}
        }
    }
    catch (e){
        console.log(e)
        return {
            code: 2,
            message: "Geçersiz stil girdiniz. Stillerin listesi:\n\n" + Object.keys(SDXLStyles).join("\n")
        }
    }
    let imagineRequest = new ImagineRequest(stylisedPrompt.positive)
    ...
    let queue = (await getQueue()).length
    await addQueue(new DatabaseParams(imagineRequest, generationOptions.positivePrompt, generationOptions.negativePrompt || "", generationOptions.styles), chatid, messageid, userid, groupid, platform, generationOptions.locale)
    if (queue === 0){
        return {
            code: 0,
