...
export const database = new sqlite.Database(
	path.join(__dirname, "../../mainDb.sqlite")
);
...
class DBCache {
	async getUser(targetUser: string): Promise<User> {
		let QRes = (await QueryDB(
			`SELECT * FROM users WHERE user_id = '${targetUser}'`
		)) as User[];
	...
	async banUser(userid: string) {
		await ExecuteDB(
			`UPDATE users SET users.banned = not users.banned WHERE user_id = '${userid}';`
		);
	}
	async getGroup(groupId: string): Promise<Group | null> {
		let dbRes = (await QueryDB(
			`SELECT * FROM servers WHERE group_id = '${groupId}';`
		)) as Group[];
		if (dbRes.length !== 0) {
			return dbRes[0];
		}
	...
	async getRole(roleNameOrId: string | number): Promise<Role> {
		if (typeof roleNameOrId === "string") {
			return (
				(await QueryDB(
					`SELECT * FROM roles WHERE role_name = '${roleNameOrId}'`
				)) as Role[]
			)[0];
		} else {
			return (
				(await QueryDB(
					`SELECT * FROM roles WHERE role_id = '${roleNameOrId}'`
				)) as Role[]
			)[0];
		}
	}
	...
	totalTokens(user: User, dbgroup: Group | null) {
		return (
			user.daily_tokens +
			user.paid_tokens +
			(dbgroup?.group_tokens ? dbgroup.group_tokens : 0)
		);
	}
	async resetTokens() {
		await ExecuteDB(`
...
`);
	...
	async useToken(
		...
		if (
			!force &&
			user.daily_tokens + user.paid_tokens + (dbgroup?.group_tokens || 0) <
				tokenAmount
		)
			return false;
...
async function clearExpiredCooldowns(cooldown: number) {
	...
	const sql = `DELETE FROM cooldownQueue WHERE sentDate < ${
		currDateSeconds - cooldown
	};`;
	await ExecuteDB(sql);
}
...
async function setRole(userId: string, targetRole: number) {
	await ExecuteDB(
		`UPDATE users SET role_id = ${targetRole} WHERE user_id = '${userId}';`
	);
