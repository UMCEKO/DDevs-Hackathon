...
import {changeUserToken, getNextPrompt, Queue, removeQueue} from "../fn/dbfn";
...
async function startImageGeneration(bool: boolean){
  while (bool) {
    let prompt: Queue = await getNextPrompt()
    if (prompt) {
      if (prompt.platform !== "discord") {
        await sleep(1000)
        continue
      }
      let guild = await client.guilds.fetch(prompt.groupid)
      let channel = await guild.channels.fetch(prompt.chatID) as TextChannel | null
      if (!channel) continue
      if (!channel.isTextBased) continue

      try {
        let dbParam = JSON.parse(Buffer.from(prompt.prompt.buffer).toString("utf-8")) as DatabaseParams

        let img
        try {
            img = await generateImage(dbParam.imagineRequest)
            let fileName = Date.now()
            let user = await client.users.fetch(prompt.author)
            let image: Buffer
            ...
            await channel.send({embeds: [
              prompt.locale === "tr" ?
                  new EmbedBuilder()
                      ...
                      .setAuthor({name: user.username, iconURL: user.avatarURL({size: 256, extension: "png", forceStatic: true}) || undefined})
                      .addFields([
                          {
                              name: "Prompt",
          ...
          if (e?.cause?.errno === -4078){
            await channel.send({embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(`Image generation is currently undergoing maintenance. <@${prompt.author}>`)
                    .setColor("#ffc400")
              ]})
          }
          else {
          ...
          console.log(e)
            await channel.send({embeds: [
                new EmbedBuilder()
                    .setTitle("Error!")
                    .setDescription(`Encountered an error whilst generating this image. <@${prompt.author}>`)
                    .setColor("#ff0000")
              ]})
          }
        }
